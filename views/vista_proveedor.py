...
def crear_vista_proveedor(root, volver_callback):
    ...
    tk.Label(frame, text="🏢  Proveedores", font=fuente_titulo, bg="white", fg="#111111").place(x=30, y=20)

    columnas = ["id_proveedor", "nombre", "telefono", "direccion", "correo"]
    ...
    def abrir_dialogo_agregar():
        ...
        for i, campo in enumerate(campos):
            y = y_base + i * 40
            tk.Label(dialog, text=campo["etiqueta"], font=fuente_label, bg="white", anchor="w").place(x=20, y=y)
        ...
        entradas["id_proveedor"].config(state="normal")
        entradas["id_proveedor"].delete(0, tk.END)
        entradas["id_proveedor"].config(state="readonly")
        ...
        def on_guardar():
            ...
            if not datos["nombre"]:
                ...
                return
            try:
                guardar_proveedor(datos, tree, dialog, False)
                cargar_proveedores_en_treeview(tree)
                messagebox.showinfo("Éxito", "Proveedor agregado correctamente", parent=dialog)
    ...
    def abrir_dialogo_editar():
        ...
        def on_guardar():
            ...
            if not datos_nuevos["nombre"]:
                ...
                return
            try:
                guardar_proveedor(datos_nuevos, tree, dialog, True)
                cargar_proveedores_en_treeview(tree)
                messagebox.showinfo("Éxito", "Proveedor actualizado correctamente", parent=dialog)
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo guardar: {e}", parent=dialog)
    ...
    def on_eliminar():
        ...
        confirmar = messagebox.askyesno("Confirmar", "¿Está seguro que desea eliminar este proveedor?")
        if confirmar:
            try:
                eliminar_proveedor_seleccionado(tree)
                cargar_proveedores_en_treeview(tree)
    ...
    # Imagen lateral
    img_path = os.path.join("icons", "layer7.jpg")
